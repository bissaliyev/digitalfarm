...
h_now = time.hour
m_now = time.minute
print(f'Current Time: {h_now}:{m_now}')
ts1_h = 0
ts1_m = 0
te1_h = 0
te1_m = 0
...
def writeFile():
    ...
    if state == 1:
        FILE_WRITE = '/home/pi/data/weight_m_' + str(time.strftime("%Y-%m-%d")) + ' ' + str(
            time.strftime("%H:%M:%S")) + '.txt'
    elif state == 2:
        FILE_WRITE = '/home/pi/data/weight_e_' + str(time.strftime("%Y-%m-%d")) + ' ' + str(
            time.strftime("%H:%M:%S")) + '.txt'
    # check file
    # first tid
    if len(msg_list) > 1:
...
while 1:
    ...
    if ts1_h * 60 + ts1_m <= h_now * 60 + m_now <= te1_h * 60 + te1_m:
        state = 1
        sendpost = 1

    if ts2_h * 60 + ts2_m <= h_now * 60 + m_now <= te2_h * 60 + te2_m:
        state = 2
        sendpost = 1

    if (ts1_h * 60 + ts1_m <= h_now * 60 + m_now <= te1_h * 60 + te1_m) == 0 and (ts2_h * 60 + ts2_m <= h_now * 60 + m_now <= te2_h * 60 + te2_m) == 0:
