...
def write_file():
    ...
    m_now = time.month
    d_now = time.day
    ft = str(y_now) + '.' + str(m_now) + '.' + str(d_now)
    if state == 1:
        FILE_WRITE = '/home/pi/Documents/data/rfid_m_' + str(ft) + '.txt'
    elif state == 2:
        FILE_WRITE = '/home/pi/Documents/data/rfid_e_' + str(ft) + '.txt'
...
def write_file_ir():
    ...
    ft = str(y_now) + '.' + str(m_now) + '.' + str(d_now)
    if state == 1:
        FILE_WRITE = '/home/pi/Documents/data/ir_m_' + str(ft) + '.txt'
    elif state == 2:
        FILE_WRITE = '/home/pi/Documents/data/ir_e_' + str(ft) + '.txt'
...
while 1:
    ...
    if h_now * 60 + m_now >= ts1_h * 60 + ts1_m and h_now * 60 + m_now <= te1_h * 60 + te1_m:
        state = 1
        sendpost = 1
    elif h_now * 60 + m_now >= ts2_h * 60 + ts2_m and h_now * 60 + m_now <= te2_h * 60 + te2_m:
        state = 2
        sendpost = 1
    else:
    ...
    if cur - tb_t >= 2000:
        timemng()
        var = "A" + str(len(cnt) - 1) + "B" + str(ir_ch1_max + ir_ch2_max) + "C\r\n"
    ...
    # ir read
    if state > 0:
        print("sta : " + str(GPIO.input(irpin[0])) + " " + str(GPIO.input(irpin[1])) + " " + str(
            GPIO.input(irpin[2])) + " " + str(GPIO.input(irpin[3])))
    ...
    if RF_ser.inWaiting() > 0 and state > 0:
        a = 0
        try:
            inputBytes[a] = RF_ser.read()
            hexBytes[a] = inputBytes[a].hex()
        except:
            inputBytes[a] = 0
        if hexBytes[a] == "54":
            a = 0
            while RF_ser.inWaiting():
                inputBytes[a] = RF_ser.read()
                hexBytes[a] = inputBytes[a].hex()
                a = a + 1
                if a > 25:
            ...
            if a >= 5:
                for i in range(16, 24):
                    msg = msg + str(hexBytes[i])
                print(msg)
                ir_true1 = 1
                ir_true2 = 1
                for i in range(15, 16):
                    ant = ant + str(hexBytes[i])
                print('antena №' + ant)
                # if int(ant) == 2:
                # ir_t = wiringpi.millis()
                perm = 0
                num = 0
